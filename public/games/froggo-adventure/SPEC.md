...
| Boss (Zone 1) | **Bogmech** | 4 HP hover-dive boss in a locked arena at the end of Lily Pond Act 2. Top + sides damage the boss (jump on him or roll into him); the bottom 14 px stinger band ALWAYS hurts Froggo on contact, even while rolling — get under the boss and you take damage. Drops into an **enraged phase** at ≤ half HP: red pulsing aura, darker armor, white-flashing eye, shorter hover/wind-up windows, faster and more aggressive player-tracking dives. |
...
| Boss (Zone 3) | **Yeti Mech** | 5 HP bipedal mech at the end of Crazy Mountain Zone Act 2. State machine: idle (paces toward Froggo, 1.5 s; 1.1 s enraged) → telegraph (raises arms, eye glows orange, 0.8 s; 0.58 s enraged) → one of two attacks → recover (**1.5 s vulnerable** to stomp/roll; 1.1 s enraged). **Pound:** leaps up, falls onto Froggo's tracked x, lands and emits a left + right ice-blue ground shockwave (280 px/s, 1.0 s lifetime — cannot be cancelled by rolling, must be jumped). **Charge:** dashes horizontally toward Froggo at 380 px/s (460 enraged); the arena climb wall at x=8700 lets Froggo mantle above the dash line. **Enraged phase** at HP ≤ 2: faster pacing, shorter idle/telegraph/recover windows, faster charge. **Vulnerability tells**: blue eye visor with X-marks instead of red, bright yellow pulsing aura with outer pulse ring, 3 spinning yellow stars overhead, "HIT IT!" text label. Boulder-lob is reserved for a later polish pass. |
...
- **1UP heart pickup** — hand-placed bonus life. Pixel-art green heart with a "1UP" label and a pulsing halo so it stands out against any background. On contact: `lives += 1` (capped at `LIVES_CAP`), the standard "1 UP!" banner triggers, and `sfxOneUp` plays. One-shot per act run — collecting it sets `collected = true` until the next `loadAct()`. Currently one is hidden in **Zone 2 Act 1** (`x=3380, y=195`) on the high secret platform reached by the `x=3050` spring; landing on the platform cleanly is the gating skill.
...
| 2 | Temple Depths | 9500 px | Inside-the-mountain temple corridors with dark-stone tiles, golden trim, carved pillars, and torch lighting. Three **mandatory** climb walls (`x=3800` h=180, `x=6600` h=200, `x=8700` h=160 in the boss arena) — every wall is taller than max jump apex (128 px) so the climb mechanic is actually required to ascend (verified margins: 52, 72, 32 px above apex). Mid-corridor **checkpoint** at `x=4500`, four pit gaps bridged by stepping stones, an upper-path reward platform past the checkpoint, and a high-secret reward platform above climb wall 2. 6 basic + 3 Heavy Bugbots in the run-up corridor (one Bugbot moved to `x=7700` after the original arena pair was pulled). **Boss arena locks at `x > 8200` for the Yeti Mech fight** (5 HP, pound + charge attacks, enraged at ≤ half HP). Target time: 200 s. |
...
- [x] **Temple visuals v2 (2026-04-25)** — full temple-only background layer:
  - **Stone-block back wall** (parallax 0.2) with brickwork tiling (96×48 alternating-offset blocks), per-block color variation, mortar lines, and gold-painted hieroglyphs (eye, serpent, sun, pyramid) carved into ~1 in 3 blocks.
  - **Carved fluted pillars** (parallax 0.4) at 220 px spacing — denser than v1's 320 — with capitals, fluting, lit-side highlights, and feet.
  - **Wall-mounted torches** on every pillar (staggered phase) with stepped flame pixels and warm glow halos.
  - **Free-standing stone braziers** (parallax 0.75, foreground) — large lit braziers Froggo walks past, with chunky flames and strong glow halos.
  - **Stone arch overhangs** (parallax 0.65) with gold engraving and drooping vine/chain decorations.
...
- [x] **Yeti Mech tuning + vulnerability readability (2026-04-25)** — Recover window 1.1 s → **1.5 s** (1.1 s enraged), telegraph 0.58 s → 0.8 s, idle 1.2 s → 1.5 s, charge speed 480 → 380 (560 → 460 enraged), shockwave 340 px/s 1.4 s → 280 px/s 1.0 s. Vulnerable state now has a clear visual tell: bright yellow pulsing aura + outer pulse ring, blue eye visor with crossed-out "X" pixel marks, 3 spinning yellow stars overhead, and a "HIT IT!" gold label above the head. Arena Bugbots at x=8500 / x=9100 removed; only one was relocated to the 7700 corridor before the arena pit so the boss fight is solo.
- [x] **Act 2 climb walls now mandatory + temple parallax bug fixed (2026-04-25)** — Previous wall heights (h=120) put their tops below player jump apex by 8 px, letting players bypass the climb mechanic by jumping. Walls 1, 2, and 4 are now taller (h=180/200/160), with the upper plateau / corridor lowered to match (3830-5000 → y=260, 6630-7400 → y=180), reward platforms re-pinned (4750→y=180, 6700→y=100), Bugbot y values corrected for the raised ground, checkpoint y updated to 260. Optional shortcut Wall 3 + its platform removed (didn't make sense after the height changes). Also fixed a parallax-loop bug where temple visuals (back-wall blocks, pillars, torches, braziers, arches) would stop drawing after camera.x got large — `screenStart` was treating camera.x as world-space when the rendering uses screen-space; now hardcoded to -100.
- [x] **Bogmech underside hurts always (2026-04-25)** — removed the rolling exception on the boss's bottom 14 px stinger band. Top + sides damage the boss; the underside always hurts Froggo regardless of rolling state.
- [x] **Pre-boss checkpoints + Zone 2 1UP pickup (2026-04-25)** — added a checkpoint right before each boss arena (Zone 1 Act 2 `x=4980`, Zone 2 Act 2 `x=8100`, Zone 3 Act 2 `x=7900`) so dying on a boss respawns at the arena entrance instead of the act start. Added a new collectible **1UP heart** entity type (level data `oneUps` array, persistent within an act run). First one is hidden on the high secret platform in Zone 2 Act 1 — visible from the main path but requires the `x=3050` spring + a clean landing.
