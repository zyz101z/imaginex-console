...
import { drawOffice, drawPads, padDir, hint } from '../office.js';
...
class HallwayEscape extends MiniGame {
  ...
  update(dt) {
    ...
    this.spawnT -= dt; if (this.spawnT <= 0) { const pat = !this.patSpawned && this.t > 3; if (pat) this.patSpawned = true; this.cw.push({ x: rand(0.25, 0.75), k: 0, color: pick(COLORS), talk: false, speed: (pat ? 0.34 : rand(0.36, 0.5)) * this.diff, pat, name: pat ? '' : coworkerName() }); this.spawnT = rand(0.7, 1.1) / this.diff; }
    ...
    for (const c of this.cw) {
      const p = this.pos(c);
      if (c.k >= 0.92) {
        if (!c.hit && Math.abs(p.x - this.x) < (c.pat ? 100 : 85)) { c.hit = true; this.hits++; this.hurtT = 0.6; this.lastHit = c.pat ? 'THERE HE IS!' : (c.name ? c.name.toUpperCase() + ': ' : '') + pick(['QUICK QUESTION!', 'GOT A SEC?', 'HEY SOUNG!', 'YOU LOOK BUSY, ANYWAY—']); this.api.grumpy(c.pat ? GRUMPY.PAT : GRUMPY.QUICK_QUESTION, c.pat ? 'CAUGHT BY PAT' : 'CAUGHT IN HALLWAY'); if (c.pat) this.api.say('gotasec'); this.api.shake(8, 0.25); this.api.particles.text(this.lastHit, this.x, 400, { impact: true, size: 56, color: '#ff6b6b' }); }
  ...
  draw(ctx) {
    ...
    for (const c of sorted) { const p = this.pos(c); if (c.pat) drawPat(ctx, p.x, p.y, p.s * 1.05, { t: this.t, walk: true, arms: 'wave' }); else drawCoworker(ctx, p.x, p.y, p.s, { t: this.t + c.x * 10, color: c.color }); if (c.talk && c.k < 0.92) bubble(ctx, p.x - (c.pat ? 110 : 80), p.y - 300 * p.s - 60, c.pat ? 220 : 160, 50, c.pat ? PAT_QUOTES.there.text : (c.name ? c.name + ': Hey Soung...' : 'Hey Soung...'), { size: c.name ? 15 : 18 }); }
    ...
    txt(ctx, hint(this.api.engine, '← → keys, or drag', 'use the ◀ ▶ pads, or drag'), 640, 150, { size: 18, color: '#ffe600', stroke: '#111', strokeW: 3 });
