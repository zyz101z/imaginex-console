...
| paper_toss | PAPER TOSS | drag-and-release to flick memos into the bin; Pat's desk fan = wind (arrows show it); dotted preview while aiming; bin relocates per basket. 5 MEMOS, 3 baskets = win (+200 each, +200 bonus); round ends when the 3rd drops or the last memo lands (24 s safety clock) — 2026-09-02 fix: the old 11 s stopwatch ended rounds before 3 throws | each miss +3 |
| rkt_run | RKT RUN | red light / green light: HOLD (click/SPACE) to creep across the break room to the RKT tray while Pat is busy at the coffee machine ("☕ ...", real BACK-VIEW sprite, flips around to look); "hm?" warning 0.38 s, then 👀 LOOKING — any movement (momentum counts!) = caught, knocked back 200 px. Reach the tray in 12 s = +500 | caught +10 each; timeout = NO RKTs TODAY |
...
| boss (4:58) | JUST ONE MORE THING | DECLINE button GLIDES and rebounds, slowing as it takes damage; 18 hits, HP PERSISTS if a Full Soung Mode interrupts (`S.bossHp` — it used to restart at full, which felt endless); forgiving 18 px hit edge; phase 2 adds ACCEPT ALL / REPLY ALL trap buttons that shadow it; invites fly in to swat (+1500) | invite hit +4, trap +10 |
